...
static int recv_frame_pcap(uint8_t *, int, uint16_t);
static int ether_init_header(void *, int, uint8_t *, uint8_t *, u_int16_t);
static int init_frame(uint8_t *, int, u_int16_t);
static int init_A070_frame(uint8_t *, int, uint8_t *, uint8_t);
static void hpav_cast_frame(uint8_t *, u_int16_t);
static int hpav_cast_frame_A071(uint8_t *, u_int8_t *);
...
static int recv_frame_pcap(uint8_t *frame_buf, int frame_len, uint16_t mmtype) {
  struct pcap_pkthdr *pcap_header;
  u_char *pcap_data;
  uint8_t *frame_ptr, *payload_ptr;
  u_int16_t *eth_type;
  int payload_len, offset, result;
  frame_ptr = frame_buf;
  memset(frame_ptr, 0x00, frame_len);
  do {
    result = pcap_next_ex(pcap, &pcap_header, (const u_char **) &pcap_data);
    if (result < 0) {
      olsr_printf(3, "pcap_next_ex: %s", pcap_geterr(pcap));
    }
    if ((uint32_t) frame_len > (uint32_t) (pcap_header->caplen)) {
      frame_len = pcap_header->caplen;
    }
    memcpy(frame_ptr, pcap_data, frame_len);
    //olsr_printf(3, "Received length: %d\n", length);
    //olsr_printf(3, "buffer byte 7-12: %02x:%02x:%02x:%02x:%02x:%02x\n", buffer[6], buffer[7], buffer[8], buffer[9], buffer[10], buffer[11]);
    struct ether_header *eth_header = (struct ether_header *) frame_ptr;
    ...
    if ((*eth_type == ntohs(ETHERTYPE_HOMEPLUG_AV))) {
      struct hpav_frame *frame = (struct hpav_frame *) payload_ptr;
      if ((frame->header.mmtype & HPAV_MM_CATEGORY_MASK) == HPAV_MM_VENDOR_SPEC) {
        payload_ptr = frame->payload.vendor.data;
      } else {
        payload_ptr = frame->payload.public.data;
      }
      if (frame->header.mmtype == mmtype) {
        offset = payload_ptr - frame_ptr;
        return offset;
      }
    }
...
static int init_frame(uint8_t *frame_buf, int frame_len, u_int16_t mmtype) {
  ...
  uint8_t *da;
  uint8_t *sa = NULL;
  memset(frame_buf, 0x00, frame_len);
...
static int init_A070_frame(uint8_t *frame_buf, int frame_len, uint8_t *macaddr, uint8_t tsslot) {
  frame_len = init_frame(frame_buf, frame_len, 0xA070);
  uint8_t *frame_ptr = frame_buf;
  frame_ptr += frame_len;

  struct get_tone_map_charac_request *mm = (struct get_tone_map_charac_request *) frame_ptr;
  memcpy(mm->macaddr, macaddr, 6);

  frame_ptr += sizeof(*mm);
  frame_len = frame_ptr - frame_buf;
  return frame_len;
}
...
static void hpav_cast_frame(uint8_t *frame_ptr, uint16_t mmtype) {
  int i;
  switch (mmtype) {
  case 0xA039: {
  ...
  case 0xA031:
    ...
    break;
  case 0x6049: {
    struct cm_get_network_stats_confirm *mm = (struct cm_get_network_stats_confirm *) frame_ptr;
    memcpy(&plc_mac, mm->sta.infos[0].macaddr, sizeof(mm->sta.infos[0].macaddr));
  }
  }
}

static int hpav_cast_frame_A071(uint8_t *frame_ptr, uint8_t *mac) {
  float bits_per_second;
  struct get_tone_map_charac_confirm *mm =
      (struct get_tone_map_charac_confirm *) frame_ptr;
  if (mm->mstatus != 0x00) {
    olsr_printf(1, "plc_service: A070-A071 error\n");
    return 0;
  }
  ...
  olsr_printf(3, "Total bits: %d\n", total_bits);
  bits_per_second = (float) total_bits / 0.0000465;
  olsr_printf(3, "Modulation rate: %.2f bit/s\n", bits_per_second);
  for (i = 0; i < p_size_t; i++) {
    if (memcmp(p_data[i].mac, mac, 6) == 0)
      p_data[i].raw_modulation_rate = (bits_per_second* 3 / 4000000); //rate in bit/s * 3/4 /1000000: exclude TURBO FEC bits and convert in Mbits/s
  }
  return 1;
}
...
static void save_plc_mac(void) {
  olsr_printf(3, "plc_service: prima di save: %02X:%02X:%02X:%02X:%02X:%02X\n", plc_mac[0], plc_mac[1], plc_mac[2], plc_mac[3], plc_mac[4], plc_mac[5]);
...
static void save_plc_data(void) {
  ...
  for(i=0; i < p_size_t; i++) {
    ...
    if(updated) {
      olsr_printf(3, "\nplc_service: plc mac aggiornato: %02X:%02X:%02X:%02X:%02X:%02X\n", p_data[i].mac[0], p_data[i].mac[1], p_data[i].mac[2], p_data[i].mac[3], p_data[i].mac[4], p_data[i].mac[5]);
      olsr_printf(3, "plc_service: plc peer trovato e aggiornato\n");
      //print_plc_peer_neighbors();
    } else {
      olsr_printf(3, "plc_service: peer da aggiornare non trovato\n");
      olsr_printf(3, "\nplc_service: NON AGGIORNATO: %02X:%02X:%02X:%02X:%02X:%02X\n", p_data[i].mac[0], p_data[i].mac[1], p_data[i].mac[2], p_data[i].mac[3], p_data[i].mac[4], p_data[i].mac[5]);
      //print_plc_peer_neighbors();
    }
  }
}
...
int update_plc_data() {
  uint8_t frame_buf[ETH_FRAME_LEN + 4];
  int frame_len, send_result, i, offset;

  frame_len = ETH_FRAME_LEN + 4;
  frame_len = init_frame(frame_buf, frame_len, 0xA038);
  send_result = send_frame_pcap(frame_buf, frame_len);

  offset = recv_frame_pcap(frame_buf, sizeof(frame_buf) , 0xA039);
  hpav_cast_frame(frame_buf + offset, 0xA039);
  save_plc_mac();
  save_plc_data();
  ...
  for (i = 0; i < p_size_t; i++) {
    uint8_t *mac;
    mac = p_data[i].mac;
    frame_len = init_A070_frame(frame_buf, frame_len, mac, 0);
    send_result = send_frame_pcap(frame_buf, frame_len);
    offset = recv_frame_pcap(frame_buf, sizeof(frame_buf) , 0xA071);
    hpav_cast_frame_A071(frame_buf + offset, mac);
  }
  save_plc_data();

//  olsr_printf(3, "\nplc_service: plc mac: %02x:%02x:%02x:%02x:%02x:%02x\n", plc_mac[0], plc_mac[1], plc_mac[2], plc_mac[3], plc_mac[4], plc_mac[5]);
//  olsr_printf(3, "plc_service: Numero di stazioni: %d\n", p_size_t);
//  olsr_printf(3, "\nplc_service: mac STA 1: %02x:%02x:%02x:%02x:%02x:%02x\n", p_data[0].mac[0], p_data[0].mac[1], p_data[0].mac[2], p_data[0].mac[3], p_data[0].mac[4], p_data[0].mac[5]);
  return 1;
}
